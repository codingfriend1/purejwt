...
function tamperWithToken(originalToken) {
  ...
  const tamperedPayloadEncoded = Buffer.from(JSON.stringify(payload)).toString('base64')
...
describe('PureJWT Instantiation', function() {
  ...
  it('should not accept an unknown algorithm', function() {
    try {
      ...
      assert.equal(err.message, `Invalid algorithm: frosty256. Must be one of: HS256, HS384, HS512, RS256, RS384, RS512, ES256, ES384, ES512, PS256, PS384, PS512`)
    }
  })
  ...
  it('should not accept an empty secret', function() {
    try {
      ...
      assert.instanceOf(err, PureJWT.PureJWTError)
      assert.equal(err.message, `A synchronous algorithm must provide a secret.`)
    }
  })
  ...
  it('should not accept a RS256 algorithm without a privateKey and PublicKey', function() {
    try {
      const jwt2 = new PureJWT({
        algorithm: 'RS256'
      })
      ...
      assert.instanceOf(err, PureJWT.PureJWTError)
      assert.equal(err.message, `An asynchronous algorithm must provide a 'publicKey' and 'privateKey'.`)
    }
  })
...
describe(`PureJWT Core Functionality`, function() {
  ...
  it('should verify a token', function() {
    ...
    assert.equal(verifiedPayload.iss, `https://securetoken.hostluxe.com/project/581753`)
  })
  ...
  it('should not create a token without a payload', function() {
    try {
      ...
      assert.instanceOf(err, PureJWT.PureJWTError)
      assert.equal(err.message, `Payload must be an object with at least one key.`)
    }
  })
  ...
  it('should fail when HS256 payload is tampered with', function() {
    ...
    const jwt2 = new PureJWT({
      secret: '7a9a66475c4d177392bd8aa4cc1f9145f494d6b2939f9b2c36687e4794ec91e5',
      acceptableIssuers: `https://securetoken.hostluxe.com/project/581753`
    })
...
describe(`'iss' (Issuer) claims`, function() {
  ...
  it(`should accept a string 'iss'`, function() {
    const payload = { sub: 'd2bb6a8d', iss: `https://securetoken.hostluxe.com/project/581753`, role: 'user' }
    const token = jwt.createToken(payload)
    const returnedPayload = jwt.verifyToken(token)
  ...
  it(`should accept a string array 'iss'`, function() {
    const iss = [`https://securetoken.hostluxe.com/project/581753`, 'https://chat.myenterprisesolution.com']
  ...
  it(`should reject a non-string 'iss'`, function() {
    ...
    try {
      ...
      assert.instanceOf(err, PureJWT.PureJWTError)
      assert.equal(err.message, `'iss' must be a String or an array of Strings.`)
    }
  })
  ...
  it(`should accept one of many valid 'acceptableIssuers'`, function() {
    ...
    const jwt2 = new PureJWT({
      secret: '7a9a66475c4d177',
      acceptableIssuers: [`https://securetoken.hostluxe.com/project/581753`, 'https://client.myretailapp.com']
    })
  ...
  it(`should accept one of many valid 'iss'`, function() {
    ...
    const payload = { sub: 'd2bb6a8d', iss: ['https://client.myretailapp.com', 'a-third-entity.com'] }
    const token = jwt2.createToken(payload)
    const returnedPayload = jwt2.verifyToken(token)
  ...
  it(`should reject a token without a 'iss' and 'acceptableIssuers' overlap`, function() {
    ...
    const payload = { sub: 'd2bb6a8d', iss: ['api.myhealthcareapp.com', 'mysocialnetwork.com'] }
    const token = jwt2.createToken(payload)
    try {
      const returnedPayload = jwt2.verifyToken(token)
      assert.fail('Expected an error to be thrown')
...
describe(`'aud' (Audience) claims`, function() {
  ...
  it(`should accept a string 'aud' (Audience)`, function() {
    const payload = { sub: 'd2bb6a8d', aud: 'https://microservice.myenterprisesolution.com' }
    const token = jwt.createToken(payload)
    const returnedPayload = jwt.verifyToken(token)
  ...
  it(`should accept a string array 'aud'`, function() {
    const aud = ['https://microservice.myenterprisesolution.com', 'https://chat.myenterprisesolution.com']
  ...
  it(`should refuse a non-string 'aud'`, function() {
    ...
    try {
      ...
      assert.instanceOf(err, PureJWT.PureJWTError)
      assert.equal(err.message, `'aud' must be a String or an array of Strings.`)
    }
  })
  ...
  it(`should reject a token without a 'aud' and 'acceptableAudiences' overlap`, function() {
    ...
    const payload = { sub: 'd2bb6a8d', aud: ['mid-tier-api', 'everything-api'] }
    const token = jwt2.createToken(payload)
    try {
      const returnedPayload = jwt2.verifyToken(token)
      assert.fail('Expected an error to be thrown')
...
describe(`'iat' (Issued At) Claims`, function() {
  ...
  it(`should reject a future 'iat'`, function() {
    ...
    const payload = { sub: 'd2bb6a8d', iat: Math.floor(Date.now() / 1000) + (60 * 60) }
...
describe('RSA PureJWT', function() {  
  ...
  it('should throw error when signature is tampered', function() {
    ...
    // Create the token with original payload
    const originalPayload = { sub: '1234567890', name: 'John Doe', role: 'user' }
